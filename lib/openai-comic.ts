...
const COMIC_VISUAL_PRODUCTION_LOCKS = [
  ...
  "- When Muci, Nia, Snacri, Padaruna, or Padarana appear together, use the similar-character comparison reference and keep their black-and-white identities separate: Muci is the broad squat model-sheet droplet with a subtle near-center reader-left top lean and no brow, Nia is the sharper tall vertical point with one angled brow, Snacri is the fatter left-leaning quiet droplet, Padaruna is the sharp point with fuller rounder buoyant lively body, no eyebrows, eager open-dot-eye smile, and about 1.3x Muci's overall size when she appears with Muci, and Padarana is the sharp point with slimmer gentle body and closed smiling eyes.",
  "- Padaruna exact lock: draw Padaruna from comic/characters/padaruna/refs/model-sheet.jpg as a sharp pointed head over a noticeably fuller round buoyant white body, open lively dot eyes, eager smile, no eyebrows or brow marks, and energetic social expression. When Padaruna appears with Muci, make Padaruna about 1.3x Muci's overall body scale and visual mass. Do not turn Padaruna into Muci's squat soft protagonist droplet, Muci's face spacing, Muci's baseline smile, Nia's angled brow, Padarana's slim gentle body, or a generic teardrop.",
  ...
  if (slugs.has("padaruna")) {
    locks.push(
      [
        ...
        "- If Padaruna appears in the same episode or page as Muci, compare them before final render: Muci stays the smaller squat soft-sided protagonist droplet; Padaruna stays about 1.3x Muci's overall size, fuller, rounder, sharper-headed, browless, and socially energetic."
      ].join("\n")
    );
  }
